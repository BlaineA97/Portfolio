...
#### React Forms: Write an Input Event Handler
  *  We will define a function that gets called whenever a user enters or deletes any character.
...
#### React Forms: Update an Input's Value
  ...
  * Easy enough! You can control an `<input />`'s text by setting its value attribute.
    * Example (of a correctly formatted React Form):
    ~~~
    import React from 'react';
    import ReactDOM from 'react-dom';
